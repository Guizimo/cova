...
<table>
  ...
  <tr>
    <td width="50%">
      <h3>🚀 导出与模板</h3>
      <ul>
        <li><strong>多格式导出</strong> — 支持 PNG、JPEG、WebP，默认 2 倍高清输出（AVIF 暂回退为 PNG）</li>
        <li><strong>模板系统与模板市场</strong> — 最多保存 10 个模板，并可从内置模板市场一键导入</li>
...
Cova 内置 AI 助手，并通过 **自带密钥（BYOK）** 的方式延续「本地优先、隐私友好」的理念——请求由你的浏览器直连你所配置的 OpenAI 兼容端点（OpenAI、OpenRouter、Azure、本地 Ollama 等），不经过我们的服务器。

- **AI 设计副驾驶** —— 既能用一句话从零生成封面，也能在当前画布上做增量微调：Cova 会把你当前的设计作为上下文发给 AI，让它在已有基础上修改，而不是每次推倒重来。
- **一键微调** —— 内置「更鲜艳 / 深色模式 / 标题更大 / 渐变背景……」等快捷指令，点一下即基于当前设计做上下文感知的调整。
- **更大控制面** —— AI 现在可驱动排版（含斜体）、颜色、渐变、毛玻璃模糊、圆角等更多维度。
- **AI 文案** —— 把一个主题变成多个有冲击力的标题/副标题候选，一键应用。
- **AI 配色** —— 从心情或关键词生成和谐的渐变 + 高对比可读文字色。
- **可取消且健壮** —— 进行中的请求可随时取消，并内置超时处理。
- **非破坏式** —— 每个 AI 结果都可一键撤销。
- **平台无关** —— 兼容任意 OpenAI 协议的 Chat Completions 端点；密钥仅保存在你的浏览器。
...
- [x] **v1.2.0** — 模板市场、真实 WebP 导出、高清输出、完整国际化
- [x] **v1.3.0** — AI 原生：AI 设计助手、AI 文案、AI 配色（自带密钥）
- [x] **v1.4.0** — AI 设计副驾驶：上下文感知的增量微调、一键微调指令、更大控制面、可取消请求
- [ ] **v1.5.0** — 协作功能
- [ ] **v2.0.0** — 插件系统、高级动画和特效
